// macOS traffic light (红绿灯) 对齐
// 移植自 dbx-main：在主线程读取/移动系统按钮，并返回保留宽度
...
async fn set_macos_traffic_light_position_inner(
    ...
    window
        .run_on_main_thread(move || unsafe {
            ...
            let close_rect = close.frame();
            let current_window_rect = close
                .superview()
                .map(|view| view.convertRect_toView(close_rect, None))
                .unwrap_or(close_rect);
            let current_center_y = window_height
                - (current_window_rect.origin.y + current_window_rect.size.height / 2.0);
            ...
            let reserved_button_rect = zoom
                ...
                .and_then(|button| {
                    button
                        .superview()
                        .map(|view| view.convertRect_toView(button.frame(), None))
                })
                .unwrap_or(close_window_rect);
            let button_center_y = window_height
                - (close_window_rect.origin.y + close_window_rect.size.height / 2.0);
    ...
    rx.await
        .map_err(|err| err.to_string())?
        .ok_or_else(|| "Unable to locate macOS traffic light buttons".to_string())
}
